...
const q=String(req.query.q||"latest Nigeria news").trim();
try{
const u="https://www.bing.com/news/search?q="+encodeURIComponent(q);
const x=await fetch(u,{headers:{"User-Agent":"Mozilla/5.0"},signal:AbortSignal.timeout(7000)});
if(!x.ok) throw new Error("HTTP "+x.status);
const h=await x.text(),$=cheerio.load(h),results=[];
$("a.title").each(function(){
const title=$(this).text().trim(),url=$(this).attr("href");
if(title&&url&&!results.some(r=>r.url===url))
results.push({title,url,link:url,publisher:"Bing News",publishedAt:new Date().toISOString()});
if(results.length>=15)return false;
});
res.json({engine:"Goobrow Latest News",query:q,count:results.length,updatedAt:new Date().toISOString(),results});
}catch(e){
console.error("NEWS:",e.message);
res.json({engine:"Goobrow Latest News",query:q,count:0,results:[],error:"News temporarily unavailable"});
}
});
